...
def humanPlayer(p):
    ...
    for i in range(0, len(p.hand)):
        cardstr = (f'{p.hand[i][0]} of {p.hand[i][1]}')
        print(f'{i+1}) {cardstr}')
    choice = 0
    while choice not in range(1, len(p.hand) + 1):
        choice = input('Enter number of card you wish to ask for> ')
        if choice.isnumeric():
            choice = int(choice)
    asked_card = p.hand[choice - 1][0]
    print(f'You chose the {p.hand[choice - 1][0]} of {p.hand[choice - 1][1]}!\n')
    playerlist = [x for x in players if x.name != p.name]
    playerlist = [x for x in playerlist if x.cardsInHand() > 0]
    print('Which player would you like to ask?')
    for i in range(0, len(playerlist)):
        print(f'{i+1}) {playerlist[i].name}')
    choice = 0
    while choice not in range(1, len(playerlist) + 1):
        choice = input('Enter number of player you wish to ask> ')
        if choice.isnumeric():
            choice = int(choice)
    ask(asked_card, playerlist[choice - 1], p)
